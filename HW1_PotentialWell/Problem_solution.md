...
- $(1), (2)$를 이용하여 모든  $K_0$값을 구하고 이를 바탕으로 $\psi(x)$와 $E_{hn}, E_{nn}$을 구하는 코드를 작성하였다.
...
- Well width, $V_0$, $m$ 등 각종 Constant들을 설정.

### 2. K0-K 그래프 그리기

- 위에서 구한 방정식으로 식을 세움
- 방정식을 먼저 plot을 이용하여 시각화함

### 3. 발산점 제거

- $\tan(K_0a)$, $\cot(K_0a)$의 발산점에서 발생하는 문제를 제거하기 위해 해당 부근 값을 NaN 처리하여 제거

### 4. Numerical Analysis을 통한 근찾기

- 해당 방정식의 근을 찾기 위해 MATLAB의 `fzero`를 사용하여 even과 odd 상태의 에너지 준위 찾음

### 5. $\psi(x)$계산 및 시각화

- 구한 에너지 준위를 바탕으로 대응하는 파동함수를 계산.
- 파동함수 그래프로 그림

### 6. 에너지 준위 시각화

- 파동함수, 에너지 준위, 우물을 함께 그래프로 나타냄
